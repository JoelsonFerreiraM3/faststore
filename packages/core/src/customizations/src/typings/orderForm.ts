export type OpenExtraParts = Array<{
  qty: number
  part: string
}>

export type CustomDataOpenExtraParts = {
  [key: string]: OpenExtraParts
}

export type ImprintingType = 'same' | 'diff'
export type LineType = 'repeat' | 'diff' | 'numbers' | 'notext'

export type FolderImprinting = {
  type: ImprintingType | ''
  quantity: number
  text: Array<{
    middleLine1?: string
...
export type CustomDataFolderImprinting = {
  [key: string]: FolderImprinting
}

// Below is copied from Faststore: /packages/api/src/platforms/vtex/clients/commerce/types/OrderForm.ts
...
export interface SubscriptionDataEntry {
  ...
  plan: {
    frequency: {
      interval: number
      periodicity: 'YEAR' | 'MONTH' | 'WEEK' | 'DAY' // NOSONAR
    }
    type: string
    validity: unknown
  }
